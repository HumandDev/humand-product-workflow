...
/sprint-report <team> --post jira
/sprint-report <team> --post confluence --space <SPACE_KEY> --parent <PAGE_ID>
```
...
- `--post jira` — After generating, add a summary comment on each ticket that has code activity.
- `--post confluence` — After generating, create or update a Confluence page with the full report.
...
| Sprint dates | `fields.sprint.startDate` / `endDate` | Report header |
...
- **Comentarios en Jira** — `/sprint-report <team> --post jira` (agrega resúmenes por ticket)
...
If `--post jira` was specified, add a comment on each ticket that has code activity (skip Not Started tickets):
...
If `--post confluence` was specified, create or update a Confluence page with the full report markdown.
