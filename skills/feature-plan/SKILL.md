...
**Strategy: progressive drill-down, not exhaustive scan.**
...
#### Step 3b: Read key files

Read specific files to understand patterns the feature should follow:
...
Report these concrete signals (do not invent time estimates):

| Signal | Value |
|--------|-------|
| Repos involved | (count and names) |
| Files to create | (count, list paths) |
| Files to modify | (count, list paths) |
...
- **Tasks**: ordered list of specific things to implement
  - Each task includes: action (create/modify), file path, and description
  - Reference the existing pattern/file to follow
...
1. Create `<path>` - <description> (follow pattern in `<reference_file>`)
2. Modify `<path>` - <description>
...

### Frontend - Web (humand-web)
Summary: <one line>
Depends on: Backend (needs API endpoint), Translations
...
1. Modify `locale/en/<namespace>.json` - Add keys: ...
2. Modify `locale/es/<namespace>.json` - Add keys: ...
3. All other locales - Add same keys with English fallback values
